/*
 * Edited by Cho Quan to suits what we need
 */
...
/*
 ...
 imageLoad = function(imageViewObject, url) {
	// Grab the filename
	var filename = url.split('/');
	filename = filename[filename.length - 1];
	// Try and get the file that has been previously cached
	var file = Ti.Filesystem.getFile(Ti.Filesystem.applicationDataDirectory, 'image_loader', filename);
	...
	if (file.exists()) {
		...
		var g = Ti.Filesystem.getFile(Ti.Filesystem.applicationDataDirectory, 'image_loader');
		if (!g.exists()) {
